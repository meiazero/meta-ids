...
data['Src_IP'] = data['Src_IP'] + ':' + data['Src_Port'].astype(str)
data['Dst_IP'] = data['Dst_IP'] + ':' + data['Dst_Port'].astype(str)
...
class SAGELayer(nn.Module):
    ...
    def forward(self, g_dgl, nfeats, efeats):
        ...
        with g_dgl.local_scope():
            ...
            g_dgl.ndata['h'] = F.relu(self.W_apply(th.cat([g_dgl.ndata['h'], g_dgl.ndata['h_neigh']], dim=2)))
...
class MLPPredictor(nn.Module):
    ...
    def apply_edges(self, edges):
        ...
        h_u = edges.src['h']
        h_v = edges.dst['h']
        score = self.W(th.cat([h_u, h_v], dim=1))
        return {'score': score}
